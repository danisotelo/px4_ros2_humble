...
if [ "$USER" != "root" ]; then
    ...
    if ! id -u "$USER" &>/dev/null; then
        ...
        HOME=/home/$USER
        echo "$USER:$PASSWORD" | chpasswd 2> /dev/null || echo ""
...
chown -R $USER:$USER /opt/ros/$ROS_DISTRO

# Set up shared volume
PX4_ROOT=$HOME/shared_volume
echo "Path to PX4-Autopilot is defined at $PX4_ROOT"

CATKIN_WS=$PX4_ROOT/catkin_ws
echo "Path to catkin_ws is defined at $CATKIN_WS"

WORKSPACE_DIR=${WORKSPACE_DIR:-~/${CONTAINER_NAME:-px4_ros2_humble}_shared_volume}
echo "Container name: ${CONTAINER_NAME:-px4_ros2_humble}"
echo "Setting up shared workspace in: ${WORKSPACE_DIR}"

# Create the shared volume path
mkdir -p $CATKIN_WS/src
chown -R $USER:$USER $PX4_ROOT

# Build ros-humble-ros-gz-bridge from source
cd $CATKIN_WS/src/ 
# [ ! -d "ros_gz" ] && git clone -b humble https://github.com/gazebosim/ros_gz.git
# sudo apt-get install gz-sim8-cli libgz-transport13-dev libgz-math7-dev libgz-cmake3-dev libignition-cmake2-dev -y
cd $CATKIN_WS
...
# Gazebo path for ROS 2 control
echo "export GZ_SIM_SYSTEM_PLUGIN_PATH='$CATKIN_WS/install/gz_ros2_control/lib'" >> $HOME/.bashrc
...
echo "=============================================================================================="
echo "OPEN YOUR NAVIGATOR AND TYPE: http://localhost:6080/ to see your container GUI!"
echo "=============================================================================================="
